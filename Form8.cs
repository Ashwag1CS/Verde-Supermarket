...
namespace Verde_Supermarket
{
    ...
    public partial class Form8 : Form
    {
        ...
        public Form8(string userName, string orderNumb)
        {
            InitializeComponent();
        ...
        public void ShowProduct()
        {
            ...
            con.Open();
            SqlCommand cmd = new SqlCommand("select pId,pName,pBrand,pPrice ,quantity from product where pCategory= 'snacks' ", con);
            SqlDataAdapter adapter = new SqlDataAdapter(cmd);
            DataTable snacksTable = new DataTable();
            adapter.Fill(snacksTable);

            dataGridView1.DataSource = snacksTable;
            con.Close();
        }
        ...
        { //search button
            ...
            var check = from p in dbContext.product
                        where p.pCategory == "snacks" && p.pName == sProduct
                        select p;
            if (check.Any())
            {
                SqlConnection con = new SqlConnection(ConnectionString);
                con.Open();
                SqlCommand cmd = new SqlCommand("select pID,pName,quantity,pBrand from product where pCategory='snacks'AND pName='" + sProduct + "'", con);
                SqlDataAdapter adapter = new SqlDataAdapter(cmd);
