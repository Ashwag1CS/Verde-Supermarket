...
namespace Verde_Supermarket
{
    public partial class Form7 : Form
    {
        ...
        public Form7(string userName,string orderNumb)
        {
            InitializeComponent();
        ...
        private void button3_Click_1(object sender, EventArgs e)
        {
            ...
            var check = from p in dbContext.product
                        where p.pCategory == "meat" && p.pName == sProduct
                        select p;
            if (check.Any())
            {
                SqlConnection con = new SqlConnection(ConnectionString);
                con.Open();
                SqlCommand cmd = new SqlCommand("select pID,pName,quantity,pBrand from product where pCategory='meat'AND pName='" + sProduct + "'", con);
        ...
        public void ShowProduct()
        {
            SqlConnection con = new SqlConnection(ConnectionString);
            con.Open();
            SqlCommand cmd = new SqlCommand("select pId,pName,pBrand,pPrice ,quantity from product where pCategory= 'meat' ", con);
            SqlDataAdapter adapter = new SqlDataAdapter(cmd);
            DataTable meatTable = new DataTable();
            adapter.Fill(meatTable);

            dataGridView1.DataSource = meatTable;
            con.Close();
        }
    }
}
