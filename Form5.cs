...
namespace Verde_Supermarket
{
    public partial class Form5 : Form
    {
        ...
        public Form5(string userName, string orderNumb)
        {
            InitializeComponent();
  ...
  ShowProduct();    
        ...
        private void button3_Click_1(object sender, EventArgs e)
        {
            ...
            var checkv = from p in dbContext.product
                        where p.pCategory == "vegetables" && p.pName == sProduct
                        select p;
            if (checkv.Any())
            {
                SqlConnection con = new SqlConnection(ConnectionString);
                con.Open();
                SqlCommand cmd = new SqlCommand("select pID,pName,quantity,pBrand from product where pCategory='vegetables'AND pName='" + sProduct + "'", con);
        ...
        public void ShowProduct()
        {
            SqlConnection con = new SqlConnection(ConnectionString);
            con.Open();
            SqlCommand cmd = new SqlCommand("select pId,pName,pBrand,pPrice ,quantity from product where pCategory= 'vegetables' ", con);
            SqlDataAdapter adapter = new SqlDataAdapter(cmd);
            DataTable vegetablesTable = new DataTable();
            adapter.Fill(vegetablesTable);

            dataGridView1.DataSource = vegetablesTable;
            con.Close();
        }
