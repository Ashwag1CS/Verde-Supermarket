...
namespace Verde_Supermarket
{
    public partial class Form3 : Form
    {
        ...
        public Form3(string userName, string orderNumb)
        {
            InitializeComponent();
        ...
        private void button3_Click_1(object sender, EventArgs e)
        {
            ...
            var check = from p in dbContext.product
                        where p.pCategory == "dairy" && p.pName == sProduct
                        select p;
            if (check.Any())
            {
                SqlConnection con = new SqlConnection(ConnectionString);
                con.Open();
                SqlCommand cmd = new SqlCommand("select pID,pName,quantity,pBrand from product where pCategory='dairy'AND pName='" + sProduct + "'", con);
        ...
        public void ShowProduct()
        {
            ...
            con1.Open();
            SqlCommand cmd = new SqlCommand("select pId,pName,pBrand,pPrice,quantity from product where pCategory= 'dairy' ", con1);
            SqlDataAdapter adapter = new SqlDataAdapter(cmd);
            DataTable dairyTable = new DataTable();
            adapter.Fill(dairyTable);

            dataGridView1.DataSource = dairyTable;
            con1.Close();
        }
